...
const words = [
  ...
  { t: "an", italic: true },
  { t: "den" },
  { t: "Behandlungsstuhl.", italic: true },
  { t: "Nicht" },
  { t: "in", italic: true },
  { t: "die" },
  { t: "Abrechnung.", italic: true, accent: true },
];
        ...
        <div className="pointer-events-none absolute inset-0">
          <div className="mx-auto grid h-full max-w-[1500px] grid-cols-12 px-6 sm:px-10">
            {Array.from({ length: 13 }).map((_, i) => (
              <span
                key={i}
                className="col-span-1 h-full border-l border-line/40 last:border-r"
              />
            ))}
          </div>
        </div>

        <ManifestoCounter scrollYProgress={scrollYProgress} />
...
function ManifestoCounter({
  ...
  return (
    <motion.div
      style={{ scale, rotate }}
      className="pointer-events-none absolute right-[-10vw] bottom-[-14vh] h-[90vh] w-[90vh] opacity-30"
    >
      <svg viewBox="0 0 800 800" className="h-full w-full">
        <circle cx="400" cy="400" r="380" fill="none" stroke="#304A58" strokeWidth="0.6" />
        <circle cx="400" cy="400" r="300" fill="none" stroke="#91AAB5" strokeDasharray="2 6" strokeWidth="0.6" />
        <circle cx="400" cy="400" r="220" fill="none" stroke="#304A58" strokeOpacity="0.4" strokeWidth="0.6" />
      </svg>
    </motion.div>
  );
}
